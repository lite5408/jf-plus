...
public final class NetWorkCenter {
    ...
    private static void doRequest(final RequestMethod method, final String url,
		...
		if(Global.getConfig("http.proxy") != null && Boolean.valueOf(Global.getConfig("http.proxy"))){
			HttpHost proxy = new HttpHost(Global.getConfig("http.proxy_ip"), Integer.valueOf(Global.getConfig("http.proxy_port")));  
			config = RequestConfig.custom().setProxy(proxy).setConnectionRequestTimeout(CONNECT_TIMEOUT).setConnectTimeout(CONNECT_TIMEOUT).setSocketTimeout(CONNECT_TIMEOUT).build();
		}else{
			config = RequestConfig.custom().setConnectionRequestTimeout(CONNECT_TIMEOUT).setConnectTimeout(CONNECT_TIMEOUT).setSocketTimeout(CONNECT_TIMEOUT).build();
		}
