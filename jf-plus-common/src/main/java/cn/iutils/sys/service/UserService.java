...
//		}
	...
	public Set<String> findRoles() {
		User user = UserUtils.getLoginUser();
		if (user == null || StringUtils.isBlank(user.getRoleGroupIds())) {
			return Collections.emptySet();
		}
		return roleService.findRolesByGroups(Arrays.asList(user.getRoleGroupIds().split(",")));
		//		return roleService.findRoles(user.getRoleIds().toArray(new String[0]));
	}
	...
	public Set<String> findPermissions() {
		...
		return roleService.findPermissionsByGroups(Arrays.asList(user.getRoleGroupIds().split(",")));
		//		return roleService.findPermissions(user.getRoleIds().toArray(
		//				new String[0]));
	}
	...
	public User getEntityByPhone(String phone){
		return dao.getEntityByPhone(phone);
	}
	...
	public User getByUserName(String userName,String userSource){
		return dao.getUserByUserNameAndSource(userName, userSource);
	}

	public User getByUserNameOrNo(String no,String username){
		return dao.getByUserNameOrNo(no,username);
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public Result batchSave(List<User> successList) {
