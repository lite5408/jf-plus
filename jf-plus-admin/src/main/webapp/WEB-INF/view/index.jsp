...
${fnc:getConfig("iutils.name")}
    ...
    <div class="left-sidebar">
        ...
        <div class="tpl-sidebar-user-panel">
            <div class="tpl-user-panel-slide-toggleable">
                <div class="tpl-user-panel-profile-picture">
                    <img src="${loginUser.photoUrl}<c:if test="${empty loginUser.photoUrl}">${ctxStatic }/assets/img/head.jpeg</c:if>" alt="用户头像">
                </div>
				<div class="am-dropdown" data-am-dropdown>
				  <button class="am-btn am-btn-sm am-btn-link am-dropdown-toggle" data-am-dropdown-toggle style="text-decoration: none;"><i class="am-icon-circle-o am-text-success tpl-user-panel-status-icon"></i> ${empty loginUser.name ? loginUser.username : loginUser.name} <span class="am-icon-caret-down"></span></button>
				  <ul class="am-dropdown-content">
				    <li><a class="am-text-sm" href="#${ctx}/user/userInfo" onclick="link('${ctx}/user/userInfo')">个人信息</a></li>
				    <li><a class="am-text-sm" href="#${ctx}/user/${loginUser.id }/changePassword" onclick="link('${ctx}/user/${loginUser.id }/changePassword')">修改密码</a></li>
				  </ul>
				</div>
            </div>
        </div>
        ...
        <ul class="sidebar-nav">
            <li class="sidebar-nav-heading" onclick="javascript:location.href='${ctx}'" style="cursor: pointer;">我的首页</li>
                ...
                <c:if test="${menu1.parentId==1}">
                    <li class="sidebar-nav-link">
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="#${ctx}/${menu1.url}" class="menu-link menu-link-clear active" level="1">
                                    <img src="${ctxStatic}${m.icon}<c:if test="${empty m.icon}">/assets/i/menu.png</c:if>" style="width:16px;height:16px;margin-right:8px;" /> ${menu1.name}
                                </a>
                            </c:otherwise>
                        </c:choose>
        ...
        var initPage = function(){
            ...
            if(link){
                ...
                if(level=='1'){
                    ...
                    $menu.parent().parent().prev().addClass("active").click();
                }
                $("#main-content").attr("src",link.substr(1));
            }else{
            	$('.sidebar-nav-sub-title:first').addClass("active").click();
            }
        }
        initPage();
        //点击左侧菜单
