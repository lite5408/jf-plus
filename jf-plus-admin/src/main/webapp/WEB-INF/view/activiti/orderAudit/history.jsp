                                ...
                                <table id="contentTable" class="am-table am-table-compact am-table-striped tpl-table-black">
                                    ...
                                    <c:forEach items="${page.list}" var="order" varStatus="status">
                                        <tr>
                                                ...
                                                <td>
                                                	<c:if test="${order.auditStatus == 12 or order.auditStatus == 3 or order.auditStatus == 4}">审批通过</c:if>
                                                	<c:if test="${order.auditStatus == 2}">审批拒绝</c:if>
                                                </td>
                                            <td>
                                                <div class="am-btn-group am-btn-group-xs">
                                                    <a role="oper" href="javascript:;" onclick="audit('${order.id}','${order.taskProcessId }','4')" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-search"></span> 查看</a>
                                                </div>
                                            </td>
                                        </tr>
                                    </c:forEach>
