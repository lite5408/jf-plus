//消息弹出提示
function tips(text){
	var html=$("<div class='err-tips'>"+text+"</div>")
	$("body").append(html)
	setTimeout(function(){
		$(".err-tips").fadeOut(1500);
	},1000)
}
...
function myPoints(cashPoints,outPoints,graderPoints){
	var calcPoints = 0;
	if(!isNaN(cashPoints)){
		calcPoints+= parseFloat(cashPoints);
	}
	
	if(!isNaN(outPoints)){
		calcPoints+= parseFloat(outPoints);
	}
	
	if(!isNaN(graderPoints)){
		calcPoints+= parseFloat(graderPoints);
	}
	return calcPoints;
}

function cacheItem(itemName,itemValue){
