    ...
    $.fn.treegridData = function (options, param) {
        ...
        target.getRootNodes = function (data) {
            ...
            $.each(data, function (index, item) {
                if (!item[options.parentColumn] || item[options.parentColumn] == '0' || item[options.parentColumn] == 0) {
                    result.push(item);
                }
            });
            return result;
            ...
            $.each(data, function (i, item) {
                if (item[options.parentColumn] == parentNode[options.id]) {
                    ...
                    $.each(options.columns, function (index, column) {
                        var td = $('<td></td>');
                        var showHtml = item[column.field];
                    	if(column.formatter){
                    		showHtml = column.formatter(showHtml,item);
                    	}
                        td.html(showHtml);
                        tr.append(td);
                    });
